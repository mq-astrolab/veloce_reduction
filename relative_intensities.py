...
from veloce_reduction.extraction import linalg_extract_column
...
def get_relints_single_order(sc, sr, ordpol, fppo, ordmask=None, nfib=19, slit_height=25, sampling_size=25, step_size=None, RON=0., gain=1., return_full=False, return_snr=True, debug_level=0, timit=False):
    """
    INPUT:
...
def get_relints(P_id, stripes, mask=None, sampling_size=25, slit_height=25, RON=0., gain=1., return_full=False, timit=False, debug_level=0):
    ...
    for ord in sorted(P_id.iterkeys()):
        ...
        if return_full:
            relints_ord,relints_ord_norm,fmodel_ord,modgrid_ord,snr_ord = get_relints_single_order(sc, sr, ordpol, fppo, ordmask=cenmask[ord], nfib=19, sampling_size=sampling_size, 
                                                                                                   slit_height=slit_height, RON=RON, gain=gain, return_full=return_full)
        else:
            relints_ord,relints_ord_norm,snr_ord = get_relints_single_order(sc, sr, ordpol, fppo, ordmask=cenmask[ord], nfib=19, sampling_size=sampling_size, slit_height=slit_height, 
                                                                            RON=RON, gain=gain, return_full=return_full)
    ...
    #get weighted mean of all relints
    allsnr = np.array([])
    #allrelints = np.zeros(])
...
def get_relints_from_indices(P_id, img, stripe_indices, mask=None, sampling_size=25, slit_height=25, RON=0., gain=1., simu=False, return_full=False, timit=False, debug_level=0):
    ...
    for ord in sorted(P_id.iterkeys()):
        ...
        # fit profile for single order and save result in "global" parameter dictionary for entire chip
        if return_full:
            relints_ord,relints_ord_norm,fmodel_ord,modgrid_ord,snr_ord = get_relints_single_order(sc, sr, ordpol, fppo, ordmask=cenmask[ord], nfib=19, sampling_size=sampling_size, slit_height=slit_height,
                                                                                                   RON=RON, gain=gain, return_full=return_full)
        else:
            relints_ord,relints_ord_norm,snr_ord = get_relints_single_order(sc, sr, ordpol, fppo, ordmask=cenmask[ord], nfib=19, sampling_size=sampling_size, slit_height=slit_height, 
                                                                            RON=RON, gain=gain, return_full=return_full)
